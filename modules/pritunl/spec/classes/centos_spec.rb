...
describe 'pritunl::centos' do
  context "Install pritunl centos repo" do
    ...
    it { is_expected.to contain_file('/etc/pki/rpm-gpg/RPM-GPG-KEY-pritunl').with_content(/BEGIN PGP PUBLIC KEY BLOCK/) }
    it { is_expected.to contain_package('pritunl') }
    it { is_expected.to contain_yumrepo('pritunl').with(
      :enabled  => 1,
      :gpgcheck => 1,
